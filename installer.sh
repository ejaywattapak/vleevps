...
banner() {
  clear || true
  printf '%b\n' "${CYAN}"
  command -v figlet >/dev/null 2>&1 && figlet -f standard "VLEE AI" || {
    echo '██╗   ██╗██╗     ███████╗███████╗'
    echo '██║   ██║██║     ██╔════╝██╔════╝'
    echo '██║   ██║██║     █████╗  █████╗  '
    echo '╚██╗ ██╔╝██║     ██╔══╝  ██╔══╝  '
...
BACKUP="/root/.vlee_ai/backup-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP"
for f in "$AI" "$MENU"; do
  [[ -f "$f" ]] && cp -a "$f" "$BACKUP/"
done
...
change_key() {
  ...
  [[ -z "$k" ]] && { echo "Key kosong."; return; }
  printf '%s\n' "$k" > "$KEY_FILE"
  chmod 600 "$KEY_FILE"
  printf "${GREEN}✔ API key updated${RESET}\n"
}
...
try:
    ...
    print("\033[1;31m✖ API key not found\033[0m"); raise SystemExit
body=json.dumps({
    "model":"cx/gpt-5.6-luna",
